...
void download_process(void) {
    ...
    if (check_image_upload == 100) {
        ...
        if (upload_mode_user_selection == "SpeedEnable") {
            if (upload_mode_user_selection_voe == "VOEno") {
                if (upload_mode_user_selection_nn == "NNno") {
                    check_upload_mode = 0x100;
                }
            }
        }

        if (check_upload_mode == 0x100) {
            cmdss.clear();
       ...
       if (upload_mode_user_selection == "SpeedEnable") {
        ...
        //cmdss << ".\\image_tool\\" << flash_tool_name << " -p " << serial_port << " -f flash_ntz.bin -b " << flash_speed << " -s " << fw1_address << " -r ";
